...
function CustomerOrderIdPageIT() {
  ...
  useEffect(() => {
    if (locale === 'en') router.push(`/profile/orders/${id}`);
  }, [locale]);
  ...
  return (
    <div>
      OrderIdPage
      <div>
        <BtnCTA label="Scarica Ricevuta" onClickAction={generatePDF} />
      </div>
    </div>
  );
}

export default CustomerOrderIdPageIT;
